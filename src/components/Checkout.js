  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        <Button
          variant="contained"
          onClick={() => {  addAddress(token, newAddress.value); handleNewAddress({isAddingNewAddress: false, value: "", });}}
        >
          Add
        </Button>
        <Button
...
const Checkout = () => {
  ...
  const fetchCart = async (token) => {
    ...
    try {
      ...
      return response.data;
    } catch {  enqueueSnackbar( "Could not fetch cart details. Check that the backend is running, reachable and returns valid JSON.", {  variant: "error" }  );
      return null;
    }
  };
  ...
  const getAddresses = async (token) => {
    ...
    try {
      const response = await axios.get(`${config.endpoint}/user/addresses`, { headers: {  Authorization: `Bearer ${token}` } });
  ...
  const deleteAddress = async (token, addressId) => {
    ...
    try {
      ...
      await axios.delete( `${config.endpoint}/user/addresses/${addressId}`, {  headers: { Authorization: `Bearer ${token}` }  }  );
  
      // Update addresses by removing the deleted address
      const updatedAddresses = addresses.all.filter( (address) => address._id !== addressId );
      setAddresses({ ...addresses, all: updatedAddresses });
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      if (response.data.success) {
        // Updating user's balance to reflect the checkout amount
        const totalCartValue = getTotalCartValue(items);
        const userBalance = parseFloat(localStorage.getItem("balance"));
        const updatedBalance = userBalance - totalCartValue;
        localStorage.setItem("balance", updatedBalance.toString());
  
        // Redirecting  to the Thanks page with a success message
        history.push("/thanks");
        ...
        enqueueSnackbar("Checkout failed. Please try again later.", { variant: "error" });
        return false;
      }
    } catch (error) {
               ...
               {addresses.all.length > 0 ? (
                addresses.all.map((address) => (
                  <Box key={address._id} my="1rem"  >
                    <Stack direction="row" alignItems="center" className={`address-item ${addresses.selected === address._id ? "selected" : "not-selected"}`} 
                          onClick={() => setAddresses({ ...addresses, selected: address._id })} >
              
                      <Typography>{address.address}</Typography>
                      <Button variant="outlined" startIcon={<Delete />} style={{ border: "none" }} onClick={() => deleteAddress(token, address._id)}>  Delete  </Button>
